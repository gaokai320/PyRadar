...
class TestOSSGadget:
    def test_get_metadata(self):
        ...
        assert type(o.get_metadata()) == dict
        assert o.get_metadata()["version"] == "1.24.1"

        assert OSSGadget("numpy").get_metadata()["version"] == "1.24.2"

        assert OSSGadget("fake_package").get_metadata() == None
    ...
    def test_parse_metadata(self):
        metadata1 = OSSGadget("postbot", "0.1.0").get_metadata()
        assert OSSGadget.parse_metadata(metadata1) == []

        metadata2 = OSSGadget("postbot", "0.1.3").get_metadata()
